...
def is_subtype(sub_sig: Signature, super_sig: Signature) -> bool:
    ...
    input_compatible = all(
        is_subtype_base(super_arg, sub_arg)
        for sub_arg, super_arg in zip(
            sub_sig.input_type, super_sig.input_type, strict=True
        )
    )
...
def is_subtype_base(sub_type: type, super_type: type) -> bool:
    ...
    if hasattr(sub_type, "__origin__") and hasattr(super_type, "__origin__"):
        ...
        return all(
            is_subtype_base(s, t)
            for s, t in zip(sub_type.__args__, super_type.__args__)
        )
...
def get_operators_with_compatible_signature(
    target: Operator, dsl: DSL
) -> set[Operator]:
    return {
        op
        for op in dsl.terms.values()
        if is_subtype(op.signature, target.signature) and op != target
    }


def get_swap_operator_mutations(
    dsl: DSL, target: OperatorNode
) -> set[SwapOperatorMutation]:
...
def get_dependencies_with_compatible_signature_and_dependency(
    computation_graph: ComputationGraph, target: OperatorNode, target_input_idx: int
) -> set[Node]:
...
def get_swap_dependency_mutations(
    computation_graph: ComputationGraph, target: OperatorNode, target_input_idx: int
) -> set[SwapDependencyMutation]:
    compatible_ops = get_dependencies_with_compatible_signature_and_dependency(
        computation_graph, target, target_input_idx
    )
    mutations = {
        SwapDependencyMutation(target, target_input_idx, op) for op in compatible_ops
    }
    return mutations


def get_operators_with_compatible_output_type(
    dsl: DSL, required_type: type
) -> set[Operator]:
    return {
        op
        for op in dsl.terms.values()
...
def get_potential_inputs_for_operator(
    computation_graph: ComputationGraph, operator: Operator, target: OperatorNode
) -> list[list[Node]]:
    descendants = get_descendants(computation_graph, target)
...
def get_add_operator_as_dependency_mutations(
    ...
    for operator in compatible_operators:
        potential_inputs = get_potential_inputs_for_operator(
            computation_graph, operator, target
        )

        if all(potential_inputs) and len(potential_inputs) == len(
            operator.signature.input_type
        ):
            for input_combination in product(*potential_inputs):
                new_operator_node = OperatorNode(
                    func=operator, inputs=list(input_combination)
                )
                ...
                mutation = AddOperatorAsDependencyMutation(
                    dep_to_sub=swap_mutation, new_input=new_operator_node
                )
...
def get_all_swap_operator_mutations(
    cg: ComputationGraph, dsl: DSL
) -> set[SwapOperatorMutation]:
    mutations = set()
    for node in cg.operator_nodes:
        mutations |= get_swap_operator_mutations(dsl, node)
    return mutations


def get_all_swap_dependency_mutations(
    cg: ComputationGraph,
) -> set[SwapDependencyMutation]:
    mutations = set()
    for node in cg.operator_nodes:
        for input_idx in range(len(node.inputs)):
            mutations |= get_swap_dependency_mutations(cg, node, input_idx)
    return mutations


def get_all_add_operator_as_dependency_mutations(
    cg: ComputationGraph, dsl: DSL
) -> set[AddOperatorAsDependencyMutation]:
    mutations = set()
    for node in cg.operator_nodes:
        for input_idx in range(len(node.inputs)):
            mutations |= get_add_operator_as_dependency_mutations(
                cg, dsl, node, input_idx
            )
    return mutations
